...
GitHub 用户必须有已验证邮箱。代码会优先读取 primary verified email，如果没有，会读取任意 verified email；都没有时登录失败。
...
| GitHub 登录失败并提示 verified email | GitHub 账号没有可读取的已验证邮箱 | 在 GitHub 验证邮箱，并确认授权包含 `user:email` |
| LinuxDo 登录提示缺邮箱 | LinuxDo 用户资料没有返回 email | 检查应用 scope 是否包含 `email`，以及账号是否提供邮箱 |
| 登录成功后刷新变成未登录 | Cookie 域名或路由模式不一致 | 优先使用前端同域 `/api` route，并确认 production `COOKIE_SECURE = "true"` |
