...
Telegram 支持在 webhook 请求里带 `X-Telegram-Bot-Api-Secret-Token` 请求头。WeMail 会用 `TELEGRAM_WEBHOOK_SECRET` 校验这个请求头，生产环境建议必须配置。
...
| `TELEGRAM_WEBHOOK_SECRET` | Worker secret | 生产建议必需 | 校验 Telegram webhook 请求头 |
