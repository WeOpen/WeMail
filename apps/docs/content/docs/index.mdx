...
<Cards>
  <Card title="1. 从 0 到 1 快速部署" href="/docs/deploy-from-zero">
    从 Fork、Cloudflare 资源、GitHub Environment 到首次管理员和真实收件的一条龙教程。
  </Card>
  <Card title="2. 本地快速开始" href="/docs/quickstart">
    安装依赖、初始化本地 D1、生成邀请码、启动 Web 和 Worker。
  </Card>
  <Card title="3. Cloudflare 资源准备" href="/docs/cloudflare-resources">
    创建 D1、KV、Pages、Worker、Email Routing 和可选 R2。
  </Card>
  <Card title="4. Worker 配置" href="/docs/worker-config">
    确认 `wrangler.toml` 占位绑定、写入 secrets、配置 CORS、Cookie 和后台业务设置。
  </Card>
  <Card title="5. OAuth 快捷登录配置" href="/docs/oauth-login">
    配置 GitHub 和 LinuxDo OAuth 应用、回调地址、Worker secrets 和邀请码验收。
  </Card>
  <Card title="6. Email Routing" href="/docs/email-routing">
    让真实邮件进入 Worker 的 `email()` 入口，并完成收件验收。
  </Card>
  <Card title="7. Pages 前端部署" href="/docs/pages-deploy">
    构建 `apps/web`，处理 API 地址，部署到 Cloudflare Pages。
  </Card>
  <Card title="8. GitHub Actions 发布" href="/docs/github-actions">
    配置 GitHub Environments 和 secrets，通过 workflow 发布 staging / production。
  </Card>
  <Card title="9. 产品成熟度路线图" href="/docs/product-maturity">
    查看 8 个成熟产品方向、管理员总览和逐项验收标准。
  </Card>
</Cards>
...
| 通知渠道 | Webhook 控制台 | — | 通用 Webhook，或 Slack / Discord / 飞书 / 企业微信传入式 Webhook（按平台消息格式投递，规则可按渠道定向） |
...
4. 如需快捷登录，配置 GitHub / LinuxDo OAuth 应用和回调地址。
5. 部署 Worker，跑远端 D1 migrations。
6. 部署 Pages，确认前端能访问正确的 Worker API。
7. 启用 Email Routing，给创建的邮箱账号投递真实邮件。
8. 用 staging 完成冒烟，再从 `main` 发布 production。
