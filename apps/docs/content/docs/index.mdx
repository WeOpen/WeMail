...
<Cards>
  ...
  <Card title="4. OAuth 快捷登录配置" href="/docs/oauth-login">
    配置 GitHub 和 LinuxDo OAuth 应用、回调地址、Worker secrets 和邀请码验收。
  </Card>
  <Card title="5. Email Routing" href="/docs/email-routing">
    让真实邮件进入 Worker 的 `email()` 入口，并完成收件验收。
  </Card>
  <Card title="6. Pages 前端部署" href="/docs/pages-deploy">
    构建 `apps/web`，处理 API 地址，部署到 Cloudflare Pages。
  </Card>
  <Card title="7. GitHub Actions 发布" href="/docs/github-actions">
    配置 GitHub Environments 和 secrets，通过 workflow 发布 staging / production。
  </Card>
  <Card title="8. 产品成熟度路线图" href="/docs/product-maturity">
