...
这一章只做资源准备，不改业务代码。目标是把 staging 和 production 需要的 Cloudflare 资源先建出来，并把后面要写入 GitHub Environment secrets 的值记录好。
...
把 staging 和 production 的 `database_id` 都保存下来，后面分别写进 GitHub Environment secret `CLOUDFLARE_D1_DATABASE_ID`。
...
部署完成后，你会在 WeMail 后台填写它：

```text
系统设置 -> 域名设置 -> example.com
```
...
R2 bucket 名不是 secret，可以提交；D1 / KV 的真实 ID 不要提交。
...
资源准备完后，继续配置 [Worker 配置](/docs/worker-config)。那里会说明 `wrangler.toml` 占位绑定、GitHub Environment secrets、CORS、Cookie 和后台业务设置。
