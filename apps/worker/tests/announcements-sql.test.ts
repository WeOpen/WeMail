...
// Seed dates sit days away from every predicate edge so the comparison stays
// stable regardless of the exact execution time (no clock mocking needed).
function buildSeedRows(): AnnouncementRow[] {
  ...
  add({ title: "默认已发布" });
  add({ title: "置顶已发布", pinned: 1, published_at: "2026-08-25T00:00:00.000Z" });
  add({ title: "即将开始", start_at: "2026-09-10T00:00:00.000Z", end_at: "2026-09-20T00:00:00.000Z" });
  add({ title: "进行中窗口", start_at: "2026-09-01T00:00:00.000Z", end_at: "2026-09-30T00:00:00.000Z" });
  add({ title: "已结束", end_at: "2026-08-01T00:00:00.000Z" });
  add({ title: "已归档", status: "已归档" });
  add({ title: "仅管理员", audience: "管理员" });
  add({ title: "仅普通成员", audience: "普通成员" });
  add({ title: "安全提醒类型", type: "安全提醒" });
  add({ title: "老公告三十天外", published_at: "2026-07-01T00:00:00.000Z" });
  ...
  for (const { label, options } of LIST_CASES) {
    it(`listPage matches the JS helper for ${label}`, async () => {
      ...
      try {
        ...
        expect(actual.announcements.map((entry) => entry.id)).toEqual(expected.map((entry) => entry.id));
