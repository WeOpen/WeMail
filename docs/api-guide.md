...
| Telegram | `GET /api/telegram/overview`、`GET/PUT /api/telegram/subscription`、`POST /api/telegram/link-code`、`POST /api/telegram/webhook`、`POST /api/telegram/test-message`、`GET /api/telegram/deliveries` |
...
6. 接入 Telegram 通知：`GET /api/telegram/overview` -> `POST /api/telegram/link-code` -> Telegram Bot webhook 调用 `POST /api/telegram/webhook` 完成自动绑定 -> `POST /api/telegram/test-message` -> `GET /api/telegram/deliveries`

Telegram 自动绑定依赖 `TELEGRAM_BOT_TOKEN`；配置 `TELEGRAM_BOT_USERNAME` 后，绑定码响应会包含可直接打开的 `https://t.me/...` deep link。生产环境建议同时配置 `TELEGRAM_WEBHOOK_SECRET`，并在 Telegram webhook secret token 请求头中校验。
